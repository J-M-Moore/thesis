...
%%
\acknowledgements{%
 A special thanks also goes out to my excellent PhD supervisor, Maria Ubiali. Not only is she an extremely accomplished researcher, with outstanding, creative ideas for novel work, she is also an absolute pleasure to work with. Her dedication to her students and postdocs is unmatched. Thank you for giving me the opportunity to work with you in such an amazing group. \\
...
Thanks go out to Alastair Toft for all of his love and support; I am extremely glad to still have you as a friend. \\
...
To my amazing friends from Downing, Evie, Mick, Stephen and Luke, I would like to thank you for all our time spent together (I know that the only reason you suffer my company is because I am the one with a copy of Agricola, but I'm willing to overlook this). \\

Thank you to Shayan, Cameron and Zahari for all the help you gave me during the start of my PhD.\\

Thanks also to the fantastic members of my research group, Luca, Maeve and Manu, who have become firm friends beyond the world of physics. \\

To those wonderful members of the department outside of the research group: Wilfred, Mitchell, Campbell, Melo, Miren and Owain. \\

%Thanks also to Philip for the brief time we spent together; it was a high point of my PhD years.\\
...
\end{equation}
with $I$ the identity matrix and $T^a$ the generators of the Lie algebra $\mathfrak{su}_{\mathbb{C}}(3)$, and $m_q$ is the mass of the quark species $q$. We define the \textit{strong coupling} $\alpha_S$ in terms of the coupling constant $g_S$ appearing in the Lagrangian density~(\ref{eq:qcdlagrangian}) via:
\begin{equation}
...
We begin by introducing \textit{structure functions} for DIS, which are the experimentally reported observables for the process. Predictions for the DIS structure functions can be written as the contraction of two tensors: the \textit{leptonic} tensor, which is perturbatively calculable, and the \textit{hadronic} tensor, which is not. We proceed to parametrise the hadronic tensor in terms of Feynman's phenomenological \textit{parton model} \cite{Feynman:1969wa}, hence introducing the central objects of study in this thesis, namely parton distributions. Next, we discuss how the parton model is improved and extended when next-to-leading order QCD effects are included. Finally, we conclude with some general remarks about techniques that have been used to \textit{prove} factorisation theorems to all orders. \\
...
\end{align}
where the pair $(i,X)$ denotes the $i$th particle in the hadronic state $X$ (with $i$ in the range $i \in \{1,...,n_X\}$, for a total of $n_X$ particles in the state $X$), the four-vector $p_{P} = (E_{P}, \vec{p}_{P})$ is the four-momentum of the particle $P$, and $\mathcal{M}(\ell H \rightarrow \ell' X)$ is the amplitude for the process. The sum over $X$ indicates a sum over all possible hadronic states which can be produced in the collision, reflecting our desire to be blind towards the hadronic products. The bar over the modulus-squared amplitude denotes the spin-sum average; in particular, we assume that all initial states are unpolarised. The notation $M_{P}$ means the rest mass of the state $P$.
...
\begin{equation}
\overline{|\mathcal{M}(\ell H \rightarrow \ell'X)|^2} = \frac{e^4}{2q^4} \textrm{Tr}\left( \gamma^{\mu} \slashed{p}_{\ell} \gamma^{\nu} \slashed{p}_{\ell'} \right) \sum_{\text{spins}}\braket{H | J_{\nu}^{\dagger} | X}\braket{X | J_{\mu} | H},
\end{equation}
where the sum over spins means a sum over the $\ket{X}$ spin states, and an average over the $\ket{H}$ spin states. At this point, it is convenient to introduce two tensors in terms of which the differential cross-section can be parametrised. We define the \textit{leptonic tensor} via:
\begin{equation}
...
H_{\mu\nu} &= \frac{1}{4\pi} \sum_{X, \text{ spins}} \left(\prod_{i=1}^{n_X} \frac{d^{d-1} \vec{p}_{(i,X)}}{(2\pi)^{d-1} 2E_{(i,X)}}\right) \cdot (2\pi)^d \delta^d\left(p_\ell + p_H - p_{\ell'} - \sum_{i=1}^{n_X} p_{(i,X)}\right)  \braket{H | J_{\nu}^{\dagger} | X}\braket{X | J_{\mu} | H},
\end{align}
where again the sum over spins means a sum over the $\ket{X}$ spin states, and an average over the $\ket{H}$ spin states. The differential cross-section may then be expressed in the simplified form:
\begin{equation}
...
H_{\mu\nu} &= \frac{1}{4\pi} \sum_{q} \int\limits_{0}^{1} \frac{d\xi}{\xi} \sum_{X, \text{ spins}} \left(\prod_{i=1}^{n_X} \frac{d^{d-1} \vec{p}_{(i,X)}}{(2\pi)^{d-1} 2E_{(i,X)}}\right) \cdot (2\pi)^d \delta^d\left(p_\ell + \xi p_H - p_{\ell'} - \sum_{i=1}^{n_X} p_{(i,X)}\right)\notag \\
&\qquad\qquad \cdot \braket{q(\xi p_H) | J_{\nu}^{\dagger} | X}\braket{X | J_{\mu} | q(\xi p_H) } f_q(\xi),
\end{align}
Here, we have replaced the hadronic state $\ket{H}$ with a state $\ket{q(\xi p_H)}$; this denotes a state comprising a free constituent $q$ of the proton,\footnote{Note we also denote the virtuality of the mediating photon by $q$; this should cause no confusion as they enter in different ways.} carrying a fraction $\xi$ of the momentum of the proton $H$. We sum over all possible constituents $q$, and we weight the contributions by some unknown probability distributions $f_q(\xi)$; these distributions represent the probability of the proton ejecting a constituent $q$ to interact with the electron, carrying a momentum fraction $\xi$. Naturally, we also integrate over all possible momentum fractions. Again, the sum over spins denotes an average over the spins of the states $\ket{q(\xi p_H)}$, and a sum over the spins of the states $\ket{X}$.

With this assumption on the form of the hadronic tensor, we may calculate the hadronic tensor at lowest order in QCD perturbation theory. In this case, we may take $\ket{X}$ to be a single quark state $\ket{X} = \ket{q(p_X)}$; then, the hadronic tensor reduces to:
\begin{align}
H_{\mu\nu} &= \frac{1}{2} \sum_{q} \sum_{\text{spins}}\int\limits_{0}^{1} \frac{d\xi}{\xi} \int\frac{d^{d-1} \vec{p}_X}{ 2E_{X}} \delta^d\left(p_\ell + \xi p_H - p_{\ell'} - p_X\right)  \braket{q(\xi p_H) | J_{\nu}^{\dagger} | q(p_X)}\braket{q(p_X) | J_{\mu} | q(\xi p_H) } f_q(\xi).
\end{align}
The phase space integral can be manipulated via:
\begin{equation}
...
which results in:
\begin{align}
H_{\mu\nu} &= \frac{1}{2} \sum_{q} \sum_{\text{spins}}\int\limits_{0}^{1} \frac{d\xi}{\xi}\ \delta((q + \xi p_H)^2) \cdot \ \braket{q(\xi p_H) | J_{\nu}^{\dagger} | q(q + \xi p_H)}\braket{q(q + \xi p_H) | J_{\mu} | q(\xi p_H) } f_q(\xi).
\end{align}
...
\end{equation}
where $x$ is the Bjorken-$x$ we introduced earlier in the text. It follows that the hadronic tensor may be expressed in the parton model as:
\begin{align}
H_{\mu\nu} &= \frac{1}{4 x q \cdot p_H} \sum_{q} \sum_{\text{spins}} \braket{q(x p_H) | J_{\nu}^{\dagger} | q(q + x p_H)}\braket{q(q + x p_H) | J_{\mu} | q(x p_H) } f_q(x).
\end{align}
We can now straightforwardly compute the matrix elements to yield:
\begin{align}
&\sum_{\text{spins}} \braket{q(x p_H) | J_{\nu}^{\dagger} | q(q + x p_H)}\braket{q(q + x p_H) | J_{\mu} | q(x p_H) } \notag\\
...
\end{align}
Applying the projectors, Eq.~\eqref{eq:hadronic_to_structure1} and Eq.~\eqref{eq:hadronic_to_structure2}, we obtain the following formulae for the structure functions:
\begin{framed}
\begin{align}
F_1(x,Q^2) &= \frac{1}{2}\sum_{q} e_q^2 f_q(x), \\[1.5ex]
F_2(x,Q^2) &= x \sum_{q} e_q^2 f_q(x).
\end{align}
\end{framed}
Note the following important features of our final structure function formulae in the parton model:
\begin{enumerate}[label = (\arabic*)]
\item We have the relation $F_2 = 2x F_1$. This identity is called the \textit{Callan-Gross relation}, and provides evidence that the primary constituents of the proton have spin-$\frac{1}{2}$; if we assume different spins for the constituents of the proton, and attempt to perform the calculations presented in this section, we obtain different relations between $F_1, F_2$ which are not observed experimentally (indeed one obtains $F_1 = 0$ for scalar quarks, for example - see the discussion below Eq.~(4.18) in \cite{Ellis:1996mzs}).
...
\item \textbf{Real and virtual gluon emission.} In this case, a gluon is emitted from either the (shown in Fig.~\ref{fig:}). 
\item \textbf{Gluon-boson fusion.} 
\end{enumerate}
...
\begin{align}
\mathcal{M}_{\mu}^i &= -i g_S \mu^{\epsilon} e_q \bar{u}(p_{X,q}) \gamma_{\mu} \frac{1}{x \slashed{p}_H - \slashed{p}_{X,g}} \slashed{\epsilon}(p_{X,g}) t^A u(xp_H)  \\[1.5ex]
i\mathcal{M}_{\mu}^f &= ,
\end{align}
where $g_S\mu^{\epsilon}$ is the coupling constant of QCD in $d$ dimensions; this is the $d$-dimensional version of the coupling $g_S$ appearing in Eq.~\eqref{eq:qcd_derivative}, but now includes an additional factor of $\mu^{\epsilon}$ where $\mu$ is an arbitrary mass scale and $\epsilon = \frac{4 - d}{2}$. 

In this case, the contribution to the hadronic tensor is given by:
\begin{align}
\label{eq:hadronic_nlo_real}
H_{\mu\nu}^{\text{NLO,\text{real}}} &= \frac{1}{4\pi} \sum_{q} \int\limits_{0}^{1} \frac{d\xi}{\xi} f_q(\xi) \int \frac{d^{d-1} \vec{p}_{q}}{(2\pi)^{d-1} 2E_{q}}  \frac{d^{d-1} \vec{p}_{g}}{(2\pi)^{d-1} 2E_{g}} \cdot (2\pi)^d \delta^d\left(q + \xi p_H - p_q - p_g\right) \cdot \sum_{r,s=i,f} \overline{\mathcal{M}_{\mu}^r \mathcal{M}_{\nu}^{s\dagger}} ,
\end{align}
where the bar on the sum of the squared amplitudes denotes the spin-colour-sum average (in particular averaging over the initial quark colours and spins). This expression can be evaluated in two steps: first, we will manipulate the phase space into a more amenable form, and then second, we shall compute the spin-sum averages of the amplitude products.
...
H_{\mu\nu}^{\text{NLO,\text{real}}} &= \frac{1}{4\pi} \sum_{q}\frac{1}{2(2\pi)^{2 - 2\epsilon}} \frac{\pi^{1-\epsilon}}{2\Gamma(1-\epsilon)} Q^{-2\epsilon} \int  du dv\ f_q\left( \frac{x}{u} \right) \frac{(1-u)^{-\epsilon}}{u^{1-\epsilon}} v^{-\epsilon} (1-v)^{-\epsilon} \sum_{r,s=i,f} \overline{\mathcal{M}_{\mu}^r \mathcal{M}_{\nu}^{s\dagger}} \\[1.5ex]
...
\noindent It remains to compute the spin-colour sum-average of the quantities quadratic in the amplitudes, $\mathcal{M}_{\mu}^r \mathcal{M}_{\nu}^{s\dagger}$.

\newpage
Virtual gluon emission can be computed similarly from first principles, but can also be incorporated by an intuitive physical argument. \\

Gluon-boson fusion proceeds similarly and, for brevity, we omit the details of the calculation in this thesis. The final answer for the structure functions at next-to-leading order in QCD is given by:


\paragraph{Proofs of factorisation.} Despite the intuitive picture that the parton model and its QCD improvement provide, we have not in any sense \textit{proved} that our factorisation theorem is valid; we have merely suggested a phenomenological model... Our proof will make use Wilson's \textit{operator production expansion} (OPE) [], and will mostly follow Schwartz []. However, it should be noted that the application of the OPE in the proof of DIS factorisation is rather limited in scope, and for more general processes slightly more sophisticated techniques are required; the relevant methods are those of the Collins-Soper-Sterman ... [], and the use of \textit{soft-collinear effective theory} (SCET) [], but these are beyond the scope of this text.
...
Having defined PDFs (at least at NLO in QCD), it will be useful to report some of their more salient properties; in this section, we describe two key features of the PDFs which shall be used throughout this thesis. First, we construct the \textit{evolution equations} for parton distributions in the $\overline{\text{MS}}$ scheme; these evolution equations give a perturbative description of the dependence of the the PDFs on the factorisation scale. Second, we prove the \textit{valence} and \textit{momentum} sum rules for PDFs. Finally, we comment on two important features of the PDFs: their \textit{positivity}, and their large-$x$ and small-$x$ \textit{scaling} behaviour.

\paragraph{Evolution equations.} Consider taking the $\mu_F^2$ derivative of the NLO structure function expression in Eq.~() above.

It is useful to note that the splitting functions which feature in these evolution equations can be determined in a process-independent way; this will be particularly useful in Chapter~\ref{chap:darkphoton}.

These evolution equations can be solved numerically. Software able to do this includes APFEL [] and EKO []; the former is used throughout the thesis with occasional modification.
...
Above, we introduced parton distributions to parametrise the non-perturbative structure of hadrons. Their very definition implies that they cannot be obtained by perturbative methods, which essentially leaves open two options for their determination: (i) lattice methods; (ii) fits to experimental data. In this text, we focus exclusively on the latter choice, which we shall describe in detail in this section.
...
F_2(x,Q^2) = ...
\end{equation}
In particular, it carries a dependence on \textit{both} the PDFs \textit{and} the strong coupling $\alpha_S$. If we were to use data on the structure function $F_2$ to extract the PDFs only, we would necessarily fix $\alpha_S(m_Z^2)$ to a particular choice. Thus our PDF set would be a PDF set produced \textit{under the assumption} that the strong coupling takes a given, fixed value at the mass of the $Z$. If we were to use this PDF set to make predictions for some new process, for consistency we would need to take the \textit{same} value for $\alpha_S(m_Z^2)$ in the corresponding hard cross-section for the process.

This can become problematic when we wish to \textit{fit} the strong coupling. For suppose that we wish to extract the value of $\alpha_S(m_Z^2)$ from some new experimental process, which involves initial-state hadrons. If we make predictions for this fit using PDFs that have been produced under the assumption $\alpha_S(m_Z^2) = \alpha_0$, for some fixed value $\alpha_0$, then we risk introducing a bias towards this value in the subsequent determination of $\alpha_S(m_Z^2)$ from the new data. This is described in significantly more detail in Ref.~\cite{Forte:2020pyp}. \\

\noindent The above argument applies equally well to \textit{any} theory parameters, not just $\alpha_S(m_Z^2)$; in this thesis, we shall be particularly concerned with theory parameters drawn from \textit{beyond the Standard Model theories} (namely \textit{dark matter models} and the \textit{Standard Model Effective Field Theory}). PDFs are usually fitted under the assumption that the SM is true, which implicitly assumes... Therefore, before we make collider predictions, we must ask ourselves:
\begin{itemize}
\item What is the error committed 
\item ... 
\end{itemize}
...
%we either parametrise a dark photon's PDF at the initial
%scale $Q_0$ by assuming a simple ansatz for the functional form of its parton distribution, 
%or
...
%(the contribution of the lepton PDFs is negligible in this work;
%we thus ignore it).
