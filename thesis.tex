...
Let us assume that we are fitting a vector of theory parameters $\vec{c} \in \mathbb{R}^{N_{\text{param}}}$ (which now may comprise PDF parameters alongside EFT parameters, for example - in Sect.~\ref{sec:quad} we worked exclusively with EFT coefficients), given data $\vec{d} \in \mathbb{R}^{N_{\text{dat}}}$. The theory predictions are a general (smooth\footnote{Technically we only require once-differentiable.}) function $\vec{t} : \mathbb{R}^{N_{\text{param}}} \rightarrow \mathbb{R}^{N_{\text{dat}}}$, and we assume that the data is normally distributed according to:
\begin{equation}
...
\end{equation}
where the $N_{\text{dat}} \times N_{\text{op}}$ Jacobian matrix $\partial \vec{t} / \partial \vec{c}$ is given by:
\begin{equation}
...
\end{align}
We now attempt to remove the $M$ dependence in this posterior. Introducing the standard \textit{Cholesky decomposition} $\Sigma = AA^T$ of the covariance matrix, where $A$ is an invertible matrix, we have that $\Sigma^{-1} = A^{-T} A^{-1}$ and hence:
\begin{equation}
...
\end{equation}
\end{framed}
For ease of notation, we shall define the \textit{tangent inverse covariance matrix} by:
\begin{equation}
...
\end{align}
Depending on the dimension of the space $\textrm{Ker}((\partial \vec{t} / \partial \vec{c})^T \Sigma^{-1})$, there are different sizes of the vector $\vec{v}_{\parallel}$. If $\dim(\textrm{Ker}((\partial \vec{t} / \partial \vec{c})^T \Sigma^{-1})) = N_{\text{dat}} - N_{\text{op}}$, which is the typical case, then $\vec{v}_{\parallel}$ has size $N_{\text{op}}$ so can absorb all the delta functions. In particular, we should be able to evaluate:
\begin{equation}
...
\noindent The key features of this posterior, as compared to the Bayesian result in Sect.~\ref{sec:interval_estimation}, are the following: 
