...
Thanks also to Philip for the lovely, if brief, time we spent together; it was a high point of my PhD years.\\
...
\begin{align}
&-\eta^{\mu\nu} H_{\mu\nu}^{\text{NLO, real}} = \frac{\alpha_S C_F  (1-\epsilon)}{2\pi \Gamma(1 - \epsilon)} \left( \frac{4\pi \mu^2}{Q^2} \right)^{\epsilon} \sum_{q} e_q^2 \int\limits_{x}^{1} \frac{du}{u} f_q\left( \frac{x}{u} \right) \bigg[ \left( \frac{2}{\epsilon^2} + \frac{3}{2\epsilon} + \frac{7}{2} \right) \delta(1-u) \notag\\[1.5ex]
...
-\eta^{\mu\nu} H_{\mu\nu}^{\text{NLO, virtual}} &= -\frac{\alpha_S C_F  (1-\epsilon)}{2\pi \Gamma(1 - \epsilon)} \left( \frac{4\pi \mu^2}{Q^2} \right)^{\epsilon} \sum_{q} e_q^2 \int\limits_{x}^{1} \frac{du}{u} f_q\left( \frac{x}{u} \right) \delta(1-u) \notag\\[1.5ex]
& \qquad\qquad\qquad\qquad\qquad \cdot \left( \frac{2}{\epsilon^2} + \frac{3}{\epsilon} + 8 + \frac{\pi^2}{3} + O(\epsilon)\right)
\end{align}
with $p^{\mu}_H p^{\nu}_H H_{\mu\nu}^{\text{NLO, virtual}}$ vanishing as $\epsilon \rightarrow 0$.\\
...
-\eta^{\mu\nu} H_{\mu\nu}^{\text{NLO, gluon}} &= ...  \\[1.5ex]
\label{eq:gluon_longitudinal_unsimplified}
p^{\mu}_H p^{\nu}_H H_{\mu\nu}^{\text{NLO, gluon}} &= ...,
\end{align}
Putting everything together then, the full projections of the hadronic tensor including both leading order and next-to-leading order terms in QCD are given by:
\begin{framed}
\begin{align}
\label{eq:full_nlo_transverse_unsimplified}
&-\eta^{\mu\nu} H_{\mu\nu}^{\text{LO+NLO}} = \frac{\alpha_S C_F  (1-\epsilon)}{2\pi \Gamma(1 - \epsilon)} \left( \frac{4\pi \mu^2}{Q^2} \right)^{\epsilon} \sum_{q} e_q^2 \int\limits_{x}^{1} \frac{du}{u} f_q\left( \frac{x}{u} \right) \bigg[ \left( -\frac{3}{2\epsilon} - \frac{9}{2}  - \frac{\pi^2}{3} \right) \delta(1-u) \notag\\[1.5ex]
&\qquad- \frac{1}{\epsilon} \frac{1 + u^2}{(1-u)_+} + 3 - u - \frac{3}{2(1-u)_+} + (1+u^2) \left( \frac{\log(1-u)}{1-u} \right)_+ - \frac{1+u^2}{1-u} \log(u) + O(\epsilon)\bigg].\\[1.5ex]
\label{eq:full_nlo_longitudinal_unsimplified}
&p^{\mu}_H p^{\nu}_H H_{\mu\nu}^{\text{LO+NLO}} = \frac{\alpha_S Q^2 C_F}{8\pi x^2} \sum_{q} e_q^2 \int\limits_{x}^{1} du f_q\left( \frac{x}{u} \right),
\end{align}
\end{framed}

\noindent Rather disturbingly, we have discovered that in the limit $\epsilon \rightarrow 0$, the hadronic tensor in the parton model at next-to-leading order in QCD is \textit{still singular}, corresponding to a collinear divergence from the real gluon emission that has not been cancelled with the divergence arising from the virtual gluon emission. It is at this point that the distributions $f_{q}(\xi)$ begin their starring role.

We initially introduced $f_q(\xi)$ as the distribution 

\paragraph{Definition of the $\overline{\text{MS}}$ PDFs.} Let us make the definition:
\begin{framed}
\begin{equation}
f_q^{\overline{\text{MS}}}(x, \mu_F^2) := f_q(x) - \frac{\alpha_S}{2\pi} \int\limits_{x}^{1} \frac{du}{u} f_q\left( \frac{x}{u}\right) \left[ P_{qq}^{(0)}(u) \left( \frac{1}{\epsilon} - \log\left( \frac{\mu^2}{\mu_F^2}\right) - \gamma + \log(4\pi) \right) \right]
\end{equation}
\end{framed}
\noindent It now appears that the distributions $f_q^{\overline{\text{MS}}}$ are singular in the limit as $\epsilon \rightarrow 0$. \textit{However}, we do not expect this to be the case, as in standard renormalisation theory.
